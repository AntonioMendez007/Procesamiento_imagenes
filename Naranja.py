...
_, thresh = cv2.threshold(gray_img2, 20, 240, cv2.THRESH_BINARY_INV+cv2.THRESH_OTSU)
...
cv2.imshow('Segmentacion de la naranja', result)
...
# Mostrar la imagen resultante
cv2.imshow('Segmentacion de la naranja LAB', result)

#----------------Segmentacion HSV--------------------------------------------
# Definir el rango de colores del fondo en HSV
lower_bound = np.array([15, 50, 50], dtype=np.uint8) # valor mínimo para H, S y V
...
# Mostrar la imagen resultante
cv2.imshow('Segmentacion de la naranja HSV', result)
